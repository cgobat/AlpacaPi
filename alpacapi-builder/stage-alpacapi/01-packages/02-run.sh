...
CORES=\$(nproc)
echo "Installing Python packages using \$CORES CPU cores"

# Detect high-core systems and optimize accordingly
if [ \$CORES -ge 16 ]; then
    echo "🔥 High-core system detected! Using aggressive parallel pip installs..."
    PIP_JOBS=\$((CORES * 2))
...
# Install basic packages first with parallel processing
pip3 install --upgrade --no-cache-dir \\
    requests flask sqlalchemy psutil pillow

# Install scientific packages that are available via pip
pip3 install --upgrade --no-cache-dir \\
    matplotlib scipy astropy \\
    pyephem astroplan photutils ccdproc \\
    astroscrappy reproject regions ginga

# Install packages that benefit from parallel compilation
pip3 install --upgrade --no-cache-dir --no-binary=all \\
